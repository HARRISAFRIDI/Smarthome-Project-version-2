...
RAPIDAPI_KEY  = "4185542166msh5b4bb520763de73p1a1293jsn8710ff899640"
...
class DeviceStatus(BaseModel):
    id: int; name: str; type: str; status: bool
    last_updated: str; energy_consumption: float

class DeviceControl(BaseModel):
    device_id: int; action: bool; reason: Optional[str] = None
...
def fetch_weather_sync(lat: float, lon: float):
    ...
    url = (f"https://{RAPIDAPI_HOST}/latlon"
           f"?latitude={lat}&longitude={lon}&lang=EN")
...
def get_live_temperature() -> float:
    ...
    cache_ok = (
        ...
        and (now - WEATHER_CACHE["fetched_at"]).total_seconds() < WEATHER_CACHE_MINUTES * 60
    )
    if cache_ok:
        return WEATHER_CACHE["temp_c"]
    temp_c, desc, city = fetch_weather_sync(lat, lon)
    if temp_c is not None:
        WEATHER_CACHE.update({"temp_c": temp_c, "description": desc, "city": city,
                               "fetched_at": now, "latitude": lat, "longitude": lon})
        return temp_c
    # Fallback
...
def compute_peak_hours():
    ...
    try:
        ...
        tbl = c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='two_week_logs'").fetchone()
        if not tbl:
            conn.close()
            logger.warning("[PEAK] No two_week_logs table yet — peak hours not computed")
            return
        rows = c.execute("""
        ...
        logger.info(f"[PEAK] Computed peak hours from DB: {sorted(PEAK_HOURS)} "
    ...
    try:
        ...
        with open(CSV_PATH) as f:
            for row in csv.DictReader(f):
                total += 1
                if row.get("ac")    == "1": counts[1] += 1
                if row.get("fan")   == "1": counts[2] += 1
                if row.get("light") == "1": counts[3] += 1
                if row.get("tv")    == "1": counts[4] += 1
                if row.get("fridge")== "1": counts[5] += 1
...
def sync_device_status_from_db():
    try:
        ...
        for did in DEVICES:
            ...
            if row: DEVICES[did]["status"] = bool(row["action"])
        conn.close()
    except Exception as e:
        logger.warning(f"[WARN] DB sync: {e}")

def push_notification(device_name: str, action: bool, reason: str, confidence: float, node: str):
    notif = {
    ...
    try:
        ...
        c.execute(
            ...
            (notif["timestamp"], device_name, notif["action"], reason, notif["confidence"], node)
        )
        conn.commit(); conn.close()
    except Exception as e:
        logger.warning(f"[NOTIFY-DB] {e}")
    logger.info(f"[NOTIFY] {device_name} -> {notif['action']} | {reason} | conf={notif['confidence']}%")
...
def log_agent_action_to_db(device_id: int, action: bool, confidence: float, reason: str):
    try:
        conn = get_db(); c = conn.cursor()
        c.execute("""
        ...
        c.execute(
            ...
            (datetime.now().isoformat(), device_id, TARGET_NAMES[device_id], int(action), confidence, reason)
        )
        conn.commit(); conn.close()
    except Exception as e:
...
def node2_rule_engine(predictions: List[Dict], now: datetime) -> List[Dict]:
    """Node 2 — Apply rules: confidence threshold, smart peak hours, night block, state check."""
    ...
    for p in predictions:
        ...
        if conf < CONFIDENCE_THRESHOLD:
            ...
            result["rule_reason"] = f"LOW_CONFIDENCE ({conf*100:.0f}% < {CONFIDENCE_THRESHOLD*100:.0f}%)"
        ...
        # Rule 3: No change needed — already in predicted state
        elif DEVICES[did]["status"] == pred:
            result["approved"]    = False
...
async def agent_loop():
    ...
    while True:
        try:
            now  = datetime.now()
            temp = await asyncio.to_thread(get_live_temperature)  # live from RapidAPI (cached 10 min)
            ...
            # ── Node 2: Rule Engine ────────────────────────────
            rule_checked = node2_rule_engine(predictions, now)
            ...
            locked = sum(1 for r in final_list if "MANUAL_LOCK" in r.get("rule_reason",""))
            node_log.append({
            ...
            node_log.append({
                ...
                "output": f"{len(actions)} devices changed" if actions else "No changes needed",
            })
            ...
            node_log.append({
                "node": "5 - Notify User",
                "output": f"{len(actions)} notifications sent" if actions else "Nothing to notify",
            })
            ...
            AGENT_STATUS["last_actions"] = [
                {"device": a["device_name"], "action": "ON" if a["predicted"] else "OFF",
                 "confidence": round(a["confidence"]*100,1)}
                for a in actions
            ]

            if actions:
                logger.info(f"[AGENT] Cycle #{AGENT_STATUS['cycle_count']}: {len(actions)} action(s) taken")
...
@app.get("/")
async def root():
    return {"message": "AI Home Automation API v4.0 — Autonomous Agent Active",
            "agent_running": AGENT_STATUS["running"], "docs": "/docs"}

@app.get("/health")
async def health():
    return {"status": "healthy", "timestamp": datetime.now().isoformat(),
            "model_loaded": RF_MODEL is not None, "agent_running": AGENT_STATUS["running"]}
...
async def get_devices_status():
    return [
        DeviceStatus(id=did, name=d["name"], type=d["type"], status=d["status"],
                     last_updated=datetime.now().isoformat(), energy_consumption=d["energy"])
...
async def get_device(device_id: int):
    ...
    d = DEVICES[device_id]; s = DEVICE_STATS.get(device_id, {})
    lock_mins = is_manually_locked(device_id)
    return {
        "device_id": device_id, "device_name": d["name"], "type": d["type"],
        "status": d["status"], "on_count": s.get("on_count",0),
        "off_count": s.get("off_count",0), "avg_accuracy": s.get("accuracy",0.90),
        "manually_locked": lock_mins is not None, "lock_minutes_remaining": lock_mins,
    }
...
async def control_device(control: DeviceControl):
    ...
    duration = control.duration_minutes if control.duration_minutes else DEFAULT_OVERRIDE_MINUTES
    ...
    logger.info(f"[MANUAL] {DEVICES[control.device_id]['name']} → {action_text} | locked for {duration}m")

    return {
        "success": True, "device_id": control.device_id,
        "device_name": DEVICES[control.device_id]["name"],
...
async def set_override_duration(req: OverrideDuration):
    ...
    return {"success": True, "device_id": req.device_id,
            ...
            "override_minutes": minutes, "minutes_remaining": remaining}

# ── Analytics ──────────────────────────────────────────────────

@app.get("/api/analytics")
async def get_analytics():
    analytics = []; total_energy = 0.0
    for did, d in DEVICES.items():
        s = DEVICE_STATS.get(did, {}); on = s.get("on_count",0)
        energy = d["energy"] * (on/100) if on > 0 else 0
        total_energy += energy
        analytics.append({
            "device_id": did, "device_name": d["name"], "type": d["type"],
            "total_actions": on + s.get("off_count",0),
            "on_count": on, "off_count": s.get("off_count",0),
            "avg_accuracy": s.get("accuracy",0.90), "energy_total": energy,
        })
    return {"timestamp": datetime.now().isoformat(),
            "total_devices": len(analytics),
            "total_energy_consumed": total_energy, "devices": analytics}
...
async def get_agent_status():
    ...
    for did, locked_at in list(MANUAL_OVERRIDES.items()):
        ...
        if remaining > 0:
            overrides[did] = {"device": TARGET_NAMES[did], "minutes_remaining": int(remaining // 60)}
    return {
...
async def current_prediction():
    ...
    temp = live_temp if live_temp is not None else round(18 + 12 * np.sin(now.hour * np.pi / 12), 2)
    temp_source = "RapidAPI Live" if live_temp is not None else "Fallback (sine-wave)"

    if RF_MODEL is None:
        return {"error": "Model not loaded"}
    features = np.array([[now.year,now.month,now.day,now.hour,now.minute,now.weekday(),
                          1 if now.weekday()>=5 else 0, temp]])
    preds = RF_MODEL.predict(features)[0]
    try:
        probas = RF_MODEL.predict_proba(features)
        confs  = [max(p[0]) for p in probas]
    ...
    for i, did in enumerate(range(1,6)):
        lock_mins = is_manually_locked(did)
        result.append({
            "device_id": did, "device_name": TARGET_NAMES[did],
    ...
    return {
        "timestamp": now.isoformat(), "hour": now.hour,
...
async def get_history(device_id: Optional[int] = None, days: int = 14):
    try:
        ...
        tbl = c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='two_week_logs'").fetchone()
        if not tbl:
            conn.close(); return {"data": [], "summary": {}}
        if device_id:
            rows = c.execute(
                "SELECT * FROM two_week_logs WHERE device_id=? ORDER BY timestamp DESC LIMIT ?",
                (device_id, days*48)).fetchall()
        else:
            rows = c.execute(
                "SELECT * FROM two_week_logs ORDER BY timestamp DESC LIMIT ?",
                (days*48*5,)).fetchall()
        data = [dict(r) for r in rows]; conn.close()
        summary = {}
        for row in data:
            did = row["device_id"]
            if did not in summary:
                summary[did] = {"device_id":did,"device_name":row["device_name"],"total_on":0,"total_off":0,"total_energy_wh":0.0}
            if row["action"]: summary[did]["total_on"] += 1
            else: summary[did]["total_off"] += 1
            summary[did]["total_energy_wh"] += row["energy_wh"]
        for did in summary:
            t = summary[did]["total_on"] + summary[did]["total_off"]
            summary[did]["on_percentage"] = round(summary[did]["total_on"]/t*100,1) if t else 0
        return {"total_records": len(data), "data": data, "summary": list(summary.values())}
    except Exception as e:
        raise HTTPException(500, str(e))
...
async def system_status():
    ...
    return {
        ...
        "model": {"type": "RandomForest", "loaded": RF_MODEL is not None,
                  "accuracy": round(acc,4), "trend": "improving"},
        "agent": {"running": AGENT_STATUS["running"],
                  "cycles": AGENT_STATUS["cycle_count"],
                  "last_cycle": AGENT_STATUS["last_cycle"]},
        "energy": {"total_consumed": round(sum(d["energy"]*0.5 for d in DEVICES.values()),2), "unit": "kWh"},
        "database": {"type": "SQLite", "records": db_rec},
        "weather": {"temp_c": WEATHER_CACHE.get("temp_c"),
    ...
    try:
        ...
        conn.execute(
            ...
            (user.name, user.email, hash_password(user.password),
             user.latitude, user.longitude, datetime.now().isoformat())
        )
        conn.commit()
        row = conn.execute("SELECT id,name,email,latitude,longitude,created_at FROM users WHERE email=?",
                           (user.email,)).fetchone()
        conn.close()
        ...
        logger.info(f"[SIGNUP] {user.name} registered. Location: ({user.latitude}, {user.longitude})")
...
async def login(creds: UserLogin):
    try:
        ...
        logger.info(f"[LOGIN] {user['name']} | Location: ({user['latitude']}, {user['longitude']})")
...
async def get_current_weather():
    ...
    return {
        ...
        "fetched_at":  WEATHER_CACHE["fetched_at"].isoformat() if WEATHER_CACHE.get("fetched_at") else None,
        "source":      "RapidAPI OpenWeather",
    }
...
async def startup_event():
    ...
    try:
        ...
        for r in rows:
            NOTIFICATIONS.append({"id": r["id"], "timestamp": r["timestamp"],
                "device": r["device"], "action": r["action"], "reason": r["reason"],
                "confidence": r["confidence"], "node": r["node"], "read": bool(r["read"])})
        conn.commit(); conn.close()
    ...
    try:
        ...
        if last:
            ...
            logger.info(f"[OK] Restored location for {last['name']}: ({last['latitude']}, {last['longitude']})")
        conn.close()
    except Exception as e:
    ...
    async def prefetch_weather():
        try:
            ...
            logger.info(f"[OK] Weather pre-fetched: {WEATHER_CACHE.get('temp_c')}°C in {WEATHER_CACHE.get('city')}")
        except Exception as e:
